...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^scorpion_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $SCORPION_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        SCORPION_BUILD=
    fi
...
function print_lunch_menu()
{
    ...
    if [ "z${SCORPION_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Scorpion model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch scorpion_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Scorpion GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function installboot()
{
    ...
    if (adb shell getprop ro.scorpion.device | grep -q "$SCORPION_BUILD");
    then
        ...
        echo "The connected device does not appear to be $SCORPION_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.scorpion.device | grep -q "$SCORPION_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
